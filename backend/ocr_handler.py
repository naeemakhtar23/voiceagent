...
class OCRHandler:
    ...
    def extract_text_from_image(self, image_path):
        ...
        try:
            ...
            # Use PSM 6 (uniform block of text) for better table detection
            # Also try with layout analysis for table detection
    ...
    def _extract_services_table(self, text):
        ...
        try:
            ...
            for i, line in enumerate(lines[:100]):  # Check first 100 lines
                ...
                if re.search(r'(?i)^services?\s*:?\s*$', line_stripped):
                    services_found = True
                    services_start_idx = i
                    logger.info(f"Found Services section at line {i}")
                    break
    ...
    def extract_text_from_image_obj(self, image):
        ...
        try:
            ...
            # Use PSM 6 for better table detection
            text = pytesseract.image_to_string(image, config='--psm 6')
            return text.strip()
        except pytesseract.TesseractNotFoundError:
            error_msg = (
